...
class BasicGenerator:
    ...
    def generate_attn(self, input_text, max_length, solver="max", use_entropy=False, use_logprob=False):
        ...
        mlp_model.load_state_dict(torch.load('./src/MLP_train/best_mlp_model.pth'))
        mlp_model.eval()
...
class AttnWeightRAG(BasicRAG):
    ...
    def keep_real_words(self, prev_text, curr_tokens, curr_hit):
        ...
        word_embeddings = {token.text: token.vector for token in doc}  # Replace with actual word embeddings retrieval
        mean_vector = np.mean(list(word_embeddings.values()), axis=0)
        real_pairs = []
        for i in range(len(tokens)):
            ...
            if match(tok):
                ...
                similarities = [cosine_similarity(embeddings, word_embeddings.get(other_tok,mean_vector)) for other_tok in tokens]
                mean_similarity = sum(similarities) / len(similarities) if similarities else 0.0
                # Combine attention weight and TF-IDF score
                tfidf_score = tfidf_scores[i] if i < len(tfidf_scores) else 0
                real_pairs.append((att * tfidf_score * mean_similarity, tok, i))
